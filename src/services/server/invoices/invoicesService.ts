...
export async function getIssuedInvoices(companyCode: string) {
  ...
  const ds = await ensureDbInitialized();
  const repo = issuedInvoiceRepositoryFrom(ds.manager);
...
export async function getStoreSummaries(
  ...
  // Raw SQL is kept to preserve behavior and performance characteristics.
  const query = `
      WITH started AS (
        SELECT 
          company_code,
  ...
  const ds = await ensureDbInitialized();
  const result = await ds.query(query, [companyCode, issuedDate]);
  ...
  type StoreSummaryRow = {
    ...
    usage_days: string;
    avg_label_count: string;
    avg_product_update_count: string;
  };

  return (result as StoreSummaryRow[]).map((row) => ({
